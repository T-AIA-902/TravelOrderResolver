...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.ministral3.modeling_ministral3 import (__name__, Callable, Optional, torch, nn, ACT2FN, Cache, GenerationMixin, use_kernel_forward_from_hub, use_kernel_func_from_hub, use_kernelized_func, FlashAttentionKwargs, BaseModelOutputWithPast, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, can_return_tuple, maybe_autocast, Ministral3Config, Ministral3PreTrainedModel, Ministral3Model, Ministral3ForCausalLM)
...
@torch.compiler.disable(recursive = False)
def Ministral3Attention_forward(
    self,
...
class Ministral3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return Ministral3Attention_forward(self, hidden_states, position_embeddings, attention_mask, past_key_values, cache_position, **kwargs)


@torch.compile(fullgraph = False, dynamic = True, options = torch_compile_options)
def Ministral3MLP_forward(self, x):
...
@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def Ministral3RMSNorm_forward(self, hidden_states):
...
class Ministral3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return Ministral3RMSNorm_forward(self, hidden_states)
...
def Ministral3RotaryEmbedding_forward(self, x, position_ids):
...
class Ministral3RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return Ministral3RotaryEmbedding_forward(self, x, position_ids)


@torch.compiler.disable(recursive = False)
@can_return_tuple
def Ministral3ForCausalLM_forward(
    self,
...
class Ministral3ForCausalLM(Ministral3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Ministral3ForCausalLM_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds, labels, use_cache, cache_position, logits_to_keep, **kwargs)
